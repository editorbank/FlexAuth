...
**_На схеме приводится взаимодействие с пользователем, как будет происходить общение между приложениями?_**

В части ключа "Субъект" может быть использовано отличительное имя сертификата (Distinguished Name), 
таким образом, если у каждого сервиса будет свой сертификат, мы сможем контролировать для них доступ.
...
## Управление кешем доступа
* Управлять правами может одновременно несколько приложений
...
## Примеры расширения возможностей Legacy-приложений.
**_Кредитный калькулятор._**
Рассмотрим возможности системы на основе приложения "Кредитный калькулятор". Это может быть простое одностраничное 
HTML-приложение. Данные для расчёта процентных ставок в зависимости от срока кредита берутся из JSON-файла размещенного
